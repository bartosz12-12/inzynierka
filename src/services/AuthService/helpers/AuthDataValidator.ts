...
export class AuthDataValidator {
  ...
  public beforeLoginValidate(loginData: ILoginDto): boolean {
    const isInstanceOfILoginDto = (object: any): object is ILoginDto => {
      return "email" in object && "password" in object;
    };
    if (!isInstanceOfILoginDto(loginData)) {
      return false;
    }
  ...
  public wasLoginSuccessful(
    apiResponse: AxiosResponse<ITokenResponseDto>
  ): boolean {
    ...
    const isInstanceOfITokenResponseDto = (
      object: any
    ): object is ITokenResponseDto => {
